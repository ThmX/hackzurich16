...
def sanitize_json(fn):
	...
	BOOL_LABELS = ["DIO_IGNITION", "MDI_EXT_BATT_LOW", "MDI_PANIC_STATE", "MDI_DTC_MIL", "MDI_RPM_OVER", "MDI_IDLE_STATE", "MDI_TOW_AWAY",
			"MDI_JOURNEY_STATE", "MDI_OVERSPEED"]
...
if(__name__ == "__main__"):
	...
	if(len(sys.argv) != 3):
		raise Exception("Wrong argument passed. Usage: sanitize_json.py infile outfile")

	sanitized = sanitize_json(sys.argv[1])
	dump = json.dumps(sanitized, cls=MyEncoder)
